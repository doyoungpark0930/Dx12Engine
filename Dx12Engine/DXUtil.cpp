...
//uAlign이 float단위인데, skinnedmesh단위로 하는게 효율이 좋을지 나중에 생각해보기
...
{
	if (indices == nullptr || indicesNum == 0)
		return;

	UINT start = 0;
	UINT end = indicesNum - 1;

	while (start < end)
	{
		UINT temp = indices[start];
...
{
	return Matrix::CreateScale(state.scale) *
