...
SRV_CONTAINER SrvManager::CreateTexture(const wchar_t* szFileName)
{
	...
	std::vector<D3D12_SUBRESOURCE_DATA> ddsSubresources; //dds에서 사용하는 Subresources
...
SRV_CONTAINER SrvManager::CreateCubemapTexture(const wchar_t* szFileName)
{
	...
	if (FAILED(LoadDDSTextureFromFile(m_device, szFileName,
		&m_textures[allocatedNum], datas, subresources)))
	{
		printf("Failed to load DDS Cubemap\n");
		throw std::exception();
	}

	UINT64 uploadBufferSize =
		GetRequiredIntermediateSize(m_textures[allocatedNum],
			0,
